...
public class BombBotMerge : MonoBehaviour
{
    ...
    {
        if(other.gameObject.CompareTag("Player") || other.gameObject.CompareTag("Robo"))
        {
            AudioManagerMerge.instance.PlaySfx(2);

            Instantiate(Explode,transform.position, Quaternion.identity);
            // anim.SetTrigger("bomb");
            PlayerControllerMerge.instance.isDead = true;
            PlayerControllerMerge.instance.Restart();
    ...
    private bool CheckDirection(Vector3 direction)
    {
        ...
        {
            if (hit.collider.CompareTag("BombBot"))
            {
                BombBotMerge bombbot = hit.collider.GetComponent<BombBotMerge>();

                //  robotwin2.MovePlayer(direction);

                if (bombbot.CheckDirection(direction))
                {
                    return true;
                }
            }
